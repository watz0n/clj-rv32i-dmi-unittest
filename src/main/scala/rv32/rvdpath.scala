...
class rvdpath extends Module {
    ...
    //when((io.imem.resp.valid&(!io.dmem.req.valid))|io.dmem.resp.valid) {
    
    val dmem_wren = (io.c2d.mem_wren === rvdp.mem_wren.Y)
    val pc_update = (io.imem.resp.valid&(!io.dmem.req.valid)) |                 // No data_port operation
                    (io.dmem.req.valid&(dmem_wren)&(io.dmem.req.ready)) |       // data_port write
                    (io.dmem.req.valid&(!dmem_wren)&(io.dmem.resp.valid))       // data_port read
    ...
    val inst = Mux(io.imem.resp.valid, io.imem.resp.data, rvspec.NOP)
    io.d2c.inst := inst
    ...
    // Use pre-defined dmem_wren 
    io.dmem.req.mfunc := Mux(dmem_wren, mram_op.MF_WR, mram_op.MF_RD)
