...
class rvdm extends Module {
    ...
    val reg_delay_mrwdata_valid = Reg(Bool())

    val reg_ddmem_req_valid = Reg(Bool())
...
//Default status
    ...
    reg_ddmem_req_valid := false.B
    io.ddmem.req.valid := reg_ddmem_req_valid
    ...
    when(io.dmi.req.bits.op === dmi_reqop.write) {
        when(sel_map(dm_addr.abstractcs)) {
        ...
        when(sel_map(dm_addr.sbdata0)) {
            ...
            //io.ddmem.req.data := dcsr_wdata
            //io.ddmem.req.valid := io.dmi.req.valid
            reg_ddmem_req_valid := true.B

            when(io.ddmem.req.ready&reg_ddmem_req_valid) {
                //io.dmi.req.ready := true.B
                reg_delay_mrwdata_valid := true.B
                reg_ddmem_req_valid := false.B
    ...
    //memory read access, need to wait for ddmem.resp.valid

    //when((io.dmi.req.bits.op === dmi_reqop.read&sel_map(dm_addr.sbdata0))|(sbcs.sbautoread & reg_mrdata_requested)) {
    when(io.dmi.req.bits.op === dmi_reqop.read&sel_map(dm_addr.sbdata0)) {
        //Only implement function for sbdata0
        //reg_sbdata(0) := dcsr_wdata
        ...
        when(reg_mrdata_requested) {

            reg_ddmem_req_valid := true.B
            when(io.ddmem.resp.valid&reg_ddmem_req_valid) {
        ...
        .otherwise {
            reg_ddmem_req_valid := true.B           
            when(reg_delay_mrwdata_valid) { //Gated autoread when host change dmi.req.addr
                reg_ddmem_req_valid := false.B
            }
            when(io.ddmem.req.ready) {
                //reg_ddmem_req_valid := false.B
    ...
    //Change handle reg_delay_mrwdata_valid by sbdata0 access
    //reg_delay_mrwdata_valid := io.ddmem.resp.valid //delay valid state when data not immediately ready
    
    //Change resp.valid policy always after req.ready by dtm.cc function void dtm_t::tick, line 561
    //io.dmi.resp.valid := io.dmi.req.valid 
    
    //io.dmi.resp.valid := Mux(reg_mrdata_requested, reg_delay_mrwdata_valid, io.dmi.req.valid)

    //Error form below: emulator: ../fesvr/dtm.cc:576: void dtm_t::tick(bool, bool, dtm_t::resp): Assertion `resp_wait' failed.
    //io.dmi.resp.valid := Mux(reg_mrdata_requested, reg_delay_mrwdata_valid, io.dmi.req.ready)

    //io.dmi.resp.valid := Mux((io.dmi.req.bits.op === dmi_reqop.read&sel_map(dm_addr.sbdata0)), //handle delayed dmi.req.ready for sbdata0 accessa, read only 
    io.dmi.resp.valid := Mux(sel_map(dm_addr.sbdata0), //handle delayed dmi.req.ready for sbdata0 access
                            reg_delay_mrwdata_valid,
                            io.dmi.req.ready)

    io.dmi.req.ready := io.dmi.req.valid //Require this assignment?! //Avoid error: emulator: ../fesvr/dtm.cc:576: void dtm_t::tick(bool, bool, dtm_t::resp): Assertion `resp_wait' failed.
